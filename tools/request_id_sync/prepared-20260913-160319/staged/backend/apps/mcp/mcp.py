...
from apps.chat.api.chat import create_chat, question_answer_inner, guarded_question
...
async def mcp_question(session: SessionDep, trans: Trans, chat: McpQuestion, request: Request):
    ...
    mcp_chat = ChatMcp(token=chat.token, chat_id=chat.chat_id, question=chat.question,
                       datasource_id=ds_id, request_id=chat.request_id)

    return await guarded_question(request.app.state.question_gate, session, session_user, mcp_chat,
