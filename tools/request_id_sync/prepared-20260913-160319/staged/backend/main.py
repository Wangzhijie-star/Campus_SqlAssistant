...
# 把还没处理过的“术语库”内容，补成 AI 能理解的向量数据。
# @SingleWorkerGuard.once--这个方法只允许执行一次。
...
# 把数据源、表、字段信息，也转成 AI 好搜索的格式。
@SingleWorkerGuard.once
def init_table_and_ds_embedding():
    fill_empty_table_and_ds_embeddings()

# 项目关闭时，把启动时占用的一些锁或资源释放掉。
...
async def lifespan(app: FastAPI):
    async with idempotency_redis_lifespan(
        ...
        settings.IDEMPOTENCY_LOCK_TTL_SECONDS, settings.IDEMPOTENCY_RENEW_INTERVAL_SECONDS,
        settings.IDEMPOTENCY_TASK_MAX_SECONDS,
    ):
        try:
            run_migrations()
            init_sqlbot_cache()
