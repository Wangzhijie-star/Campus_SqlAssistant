              ...
              >
                ...
                <template v-if="message.role === 'assistant' && !message.first_chat">
                  <ChartAnswer
                    ...
                    @request-rejected="onQuestionRejected"
                    @stop="onChatStop"
                  >
...
import { createRequestId } from '@/utils/questionRequest'
...
async function onChartAnswerFinish(id: number, replay = false) {
...
function onQuestionRejected(code: string, question?: string) {
  if ((code === 'CHAT_BUSY' || code === 'RESULT_UNAVAILABLE') && !inputMessage.value.trim() && question) {
    inputMessage.value = question
  }
}
...
const sendMessage = async (
  ...
  currentRecord.request_id = createRequestId()
