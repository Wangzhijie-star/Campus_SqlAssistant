...
from apps.chat.models.question_request import ChatQuestionBase, RequestId
...
class AiModelQuestion(BaseModel):
    ...
    # lang：用户语言 language
    # engine：数据库类型，比如 MySQL/PostgreSQL
    # schema：表结构
    # question：用户问题
    # rule：额外规则
    # current_time：当前时间字符串
    # error_msg：上次 SQL 执行错误
    # change_title：是否需要生成 brief 把这些数据塞入模板
...
class ChatQuestion(AiModelQuestion):
    chat_id: int
    # Required at question HTTP/MCP entry points; legacy internal tasks can omit it.
