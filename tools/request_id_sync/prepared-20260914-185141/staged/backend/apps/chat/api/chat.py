...
async def question_answer(session: SessionDep, current_user: CurrentUser, request_question: ChatQuestionBase,
                          current_assistant: CurrentAssistant, request: Request):
    question = ChatQuestion(chat_id=request_question.chat_id, question=request_question.question,
                            request_id=request_question.request_id)
    return await guarded_question(request.app.state.question_gate, session, current_user, question,
                                  current_assistant, embedding=True)


async def guarded_question(gate, session, current_user, question, current_assistant=None, **kwargs):
    # Match the execution layer's workspace checks before exposing lock state or acquiring keys.
    chat = session.get(Chat, question.chat_id)
    if not chat:
        raise HTTPException(status_code=404, detail='Chat not found')
    if chat.oid:
        if current_assistant:
            allowed = chat.oid == current_user.oid
        else:
            allowed = chat.oid in [item.id for item in user_ws_list(session, current_user.id)]
        if not allowed:
    ...
    try:
        ...
        async with gate.enter(question.chat_id, question.request_id, request_only=True) as lease:
            ...
            await gate.pause_for_test(lease)
...
def gate_error_response(error, request_id=None):
    return JSONResponse(status_code=error.status_code, content={
        'code': error.code, 'message': str(error), 'request_id': request_id,
    })


def collect_result(llm_service):
    result = {}
...
async def question_answer_inner(session: SessionDep, current_user: CurrentUser, request_question: ChatQuestion,
    ...
    try:
        ...
        if command:
            ...
            if command == QuickCommand.REGENERATE:
                ...
                return await analysis_or_predict(session, current_user, rec_id, 'analysis', current_assistant, in_chat,
                                                 stream, request_id=request_question.request_id)
        ...
        if stream:
            def _err(_e: Exception):
                if in_chat:
                    yield 'data:' + orjson.dumps({'content': str(_e), 'type': 'error',
                                                 'request_id': request_question.request_id}).decode() + '\n\n'
                else:
                    yield f'&#x274c; **ERROR:**\n'
                    yield f'> {str(_e)}\n'
            ...
            return JSONResponse(
                content={'message': str(e), 'request_id': request_question.request_id},
                status_code=500,
            )
...
async def stream_sql(session: SessionDep, current_user: CurrentUser, request_question: ChatQuestion,
    ...
    if stream:
        ...
        raw_data = await asyncio.to_thread(collect_result, llm_service)
        status_code = 200
...
async def analysis_or_predict_question(session: SessionDep, current_user: CurrentUser,
    ...
    chat = session.get(Chat, record.chat_id)
    if not chat or chat.oid != current_user.oid or chat.create_by != current_user.id:
        raise HTTPException(status_code=403, detail='Cannot access this chat')
    try:
        async with request.app.state.question_gate.enter(record.chat_id, None):
            return await analysis_or_predict(session, current_user, chat_record_id, action_type, current_assistant)
    except GateRejected as error:
        return gate_error_response(error)
...
async def analysis_or_predict(session: SessionDep, current_user: CurrentUser, chat_record_id: int, action_type: str,
    ...
    try:
        ...
        request_question = ChatQuestion(chat_id=record.chat_id, question=record.question, request_id=request_id)
