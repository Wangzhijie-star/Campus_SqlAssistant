...
{last_execute_sql_error}
    ...
    def generate_sql(self, _session: Session):
        ...
        #self.sql_message 是 List[BaseMessage]
    ...
    def run_task(self, in_chat: bool = True, stream: bool = True,
        ...
        try:
            ...
            # generate sql
            sql_res = self.generate_sql(_session) #self_res可迭代流
            full_sql_text = ''
            # chunk的格式稳定为 
            #{"content": str,reasoning_content": str}
            for chunk in sql_res:
                full_sql_text += chunk.get('content') #把所有的content都拼接起来，其形式是模型返回的json字符串
                if in_chat:
                    yield 'data:' + orjson.dumps(
                        {'content': chunk.get('content'), 'reasoning_content': chunk.get('reasoning_content'),
...
def process_stream(res: Iterator[BaseMessageChunk],
    ...
    for chunk in res:
        ...
        content = chunk.content #这是模型真正回答问题的内容，后续会拼接到full_sql_text的东西
        output_content = ''  # 实际要输出的内容

        # 检查additional_kwargs中的reasoning_content 提取推理内容
